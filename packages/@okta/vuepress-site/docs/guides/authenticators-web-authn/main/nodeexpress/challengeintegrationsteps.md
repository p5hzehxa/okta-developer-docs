...
If you configure your Okta org as detailed in [Configuration updates](#update-configurations) and the Passkeys (FIDO2 WebAuthn) authenticator is already [enrolled](#integrate-sdk-for-authenticator-enrollment) for the user, `authenticate()` returns a response indicating that the authenticator is required for verification. Specifically, `IdxTransaction` is returned with a `status` of `PENDING` and `nextStep.name` set to `challenge-authenticator`.
