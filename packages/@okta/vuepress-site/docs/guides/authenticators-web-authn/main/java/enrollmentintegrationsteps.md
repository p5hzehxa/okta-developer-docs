...
If you configure your Okta org as detailed in [Configuration updates](#update-configurations) and Passkeys (FIDO2 WebAuthn) authenticator isn’t already [enrolled](#integrate-sdk-for-authenticator-enrollment) for the user, `authenticate()` returns an `AuthenticationResponse` object with `authenticationStatus` equal to `AWAITING_AUTHENTICATOR_ENROLLMENT_SELECTION` and a `webauthn` authenticator item in the `authenticators` array.
...
![Image illustrating the enroll authenticator page showing the available sign-in authenticators, including the Passkeys (FIDO2 WebAuthn).](/img/authenticators/authenticators-webauthn-java-enroll-dropdown-selection.png)
...
1. The browser prompts the user to choose the type of Passkeys (FIDO2 WebAuthn) authenticator that they’re using. For example: a portable hardware authenticator such as a USB security key or a software-based authenticator such as a fingerprint scanner.

<div class="three-quarter">

![Image of the browser prompt that allows the user to pick the type of Passkeys (FIDO2 WebAuthn) authenticator to register.](/img/authenticators/authenticators-webauthn-authenticator-options.png)
